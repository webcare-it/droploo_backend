...
//            }
    ...
    public function processDropshipperSelectedOrders (Request $request)
    {
        $selectedOrderIds = $request->id;
        if($selectedOrderIds==null){
            return redirect()->back()->with('error', 'Select Minimum One!');
        ...
        $clickedButton = $request->input('submit_button');
        if ($clickedButton === 'print') {
            $selectedOrders = Order::with('orderDetails', 'admin', 'dropshipper')->whereIn('id', $selectedOrderIds)->get();
            ...
            return view('admin.dropshipper-pdf', compact('selectedOrders'));
        }
