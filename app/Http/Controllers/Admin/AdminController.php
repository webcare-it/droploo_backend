...
class AdminController extends Controller
{
    ...
    public function sliderUpdate(Request $request, $id)
    {
        ...
        if($request->hasFile('image')){
            ...
            $sliderUpdate->imageUrl = $imageUrl;
            $sliderUpdate->save();
        }
    ...
    {
        $products = Product::orderBy('created_at', 'desc')->where('is_custom', '!=', true)->get();
        return view('admin.customer.order-custom', compact('products'));
    }
    ...
    //Dropshipper Functionalities........

    public function depositList ()
    {
        $deposits = DropshipperDeposit::orderBy('id', 'desc')->with('dropshipper')->get();
        return view('admin.deposit.list', compact('deposits'));
    }
    ...
    public function depositApprove($id)
    {
        ...
        try {
                $deposit = DropshipperDeposit::findOrFail($id);
                $dropshipper = Dropshipper::findOrFail($deposit->dropshipper_id);
                ...
                $deposit->status = 1;
                $dropshipper->total_deposit += $deposit->amount;
    ...
    public function depositReject ($id)
    {
        ...
        $deposit->status = 2;
        $deposit->save();
        return redirect()->back()->with('warning', 'Rejected!');
    }
