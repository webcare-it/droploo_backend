                ...
                <div class="card radius-10 mb-0">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-4">
                                <div>
                                    <h5 class="mb-1">Dropshipper Delivered Orders</h5>
                                </div>
                            </div>
                            <div class="col-md-4"></div>
                            <div class="col-md-4">
                                <form action="{{ url('/dropshipper-order/delivery') }}" method="GET">
                                    @csrf
                                    <div class="input-group mb-3">
                                        ...
                                        <a href="{{ url('/dropshipper-order/delivery') }}"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        <form method="GET" action="{{ url('/dropshipper-order/delivery') }}"
                                            ...
                                            <div class="input-group mb-3">
                                                ...
                                                <a href="{{ url('/dropshipper-order/delivery') }}" class="btn btn-sm btn-danger"><i
                                ...
                                <table class="table table-striped table-hover align-middle mb-0">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th width="15%"> SetTrackingID(Link)</th>
                                            <th width="5%">Dropshipper</th>
                                            <th width="5%">Is Paid?</th>
                                            <th width="15%">Customer</th>
                                            <th width="15%">Product</th>
                                            <th width="20%">Total</th>
                                            <th width="5%">Status</th>
                                            <th width="5%">Steadfast Status</th>
                                            <th width="10%">Date</th>
                                        ...
                                        @foreach ($delivered_orders as $key => $order)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="{{ url('/dropshipper-order/set-tracking-link/' . $order->id) }}" class="btn btn-sm btn-info">Already Set</a><br>
                                                    <a href="{{$order->tracking_link}}" target="_blank" class="btn btn-sm btn-warning">Track</a>
                                                    @else
                                                    <a href="{{ url('/dropshipper-order/set-tracking-link/' . $order->id) }}" class="btn btn-sm btn-success">Set Tracking</a>
                                                    @endif
                                                </td>
                                                <td>{{$order->dropshipper->domain_name}}</td>
                                                <td>
                                                    @if ($order->is_dpaid == 1)
                                                    <span class="badge rounded-pill bg-success">Yes</span>
                                                    @elseif ($order->is_dpaid == 0)
                                                ...
                                                <td>
                                                    {{$order->steadfast_order_status ?? "pending"}}
                                                </td>
                                                <td>{{ date('d-m-Y', strtotime($order->created_at)) }}</td>
                                                <td>{{ Str::ucfirst($order->admin?->name) }}</td>
                                                <td>
                                                @if ($order->is_dpaid == 0)
                                                {{-- <a href="{{ url('/dropshipper-order/payment/' . $order->id) }}" onclick="return confirm('Are you sure?')" class="btn btn-sm btn-danger">Pay</a> --}}
                                                <a href="{{ url('/dropshipper-order/manual-payment/' . $order->id) }}" class="btn btn-sm btn-danger">Pay</a>
                                                @elseif ($order->is_dpaid == 1)
